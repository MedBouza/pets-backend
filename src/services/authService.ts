...
export const generateAccessToken = (user: Partial<IUserModel>) => {
   return jwt.sign({ _id: user._id, email: user.email }, process.env.ACCESS_TOKEN_SECRET as string, { expiresIn: '15m' });
};

export const generateRefreshToken = (user: Partial<IUserModel>) => {
   let refreshTokens = [];

   const refreshToken = jwt.sign({ _id: user._id, email: user.email }, process.env.REFRESH_TOKEN_SECRET as string, { expiresIn: '20m' });
